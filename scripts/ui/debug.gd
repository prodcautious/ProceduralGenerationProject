...
@onready var villagers_socializing_label : Label = $MarginContainer/VBoxContainer/VillagersSocializingLabel
...
func _process(delta: float) -> void:
	...
	for villager in villagers_in_group:
		if villager.current_state == villager.VillagerStates.WORKING:
			...
			walking_count += 1
		elif villager.current_state == villager.VillagerStates.SOCIALIZING:
			socializing_count += 1
	...
	villagers_socializing_label.text = "Socializing: " + str(socializing_count)
